...
<body>
    <form action = "index.php?controller=news&action=update&id=<?=$_GET['id']?>" method = "POST" enctype="multipart/form-data">
        <label class="form-label" for = "title">Title:</label><br>
        <input class="form-control"  type = "text" name = "title" id = "title" require value = "<?=$news[0]->getTitle()?>" ><br><br>
        <label class="form-label" for = "content">Content: </label><br>
        <textarea class="form-control" type = "text" name = "content" id = "content" require ><?=$news[0]->getContent()?></textarea><br><br>
        ...
        <input class="form-control"  type = "text" name = "category" id = "category" require value = "<?=$news[0]->getCategoryName()?>"><br><br>
